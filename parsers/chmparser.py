...
class ChmFileException(Exception):
    pass
...
class ChmParser(Parser):
    ...
    def set_filename(self, filename):
        ...
        self._chm_file = SimpleChmFile(filename=self._filename)
        self._chm_file.GetArchiveInfo()

    def title(self):
        if self._chm_file.title:
            return unicode(self._chm_file.title)
        else:
    ...
    def content(self):
        ...
        for page in self._chm_file:
            content += page

        html_parser = HtmlParser(html=content)
